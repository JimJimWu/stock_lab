...
def auto_update_industry_db(sid):
    ...
    【分流審查鐵律】（若搜尋結果不符條件，必須且只能輸出「【資料不足，無法確認】」，嚴禁拿其他欄位資訊湊數填補！）
    1. 個股主要業務【嚴格事實】：僅限確切已知的現行產品與業務，嚴禁推測詞。
    2. 產業市場規模【宏觀預估】：必須包含「整體市場的產值或預估數字」，且「強制標註機構來源(如MIC, TrendForce)」。若查無整體市場數據，絕對不准拿「公司業務簡介」來充數！
