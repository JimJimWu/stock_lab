...
# ==========================================================================
    # 💥 【V2.6 究極視覺版：雙軌分流 + 絕對 Markdown 格式鎖】
    # 找回 5/12 的完美閱讀體驗，強制 AI 採用「粗體標題 + 條列式」排版
    # ==========================================================================
    summary = f"""請以頂尖量化分析顧問的身份，精準提煉「{company_name}」的產業百科。
    【排版與分流最高鐵律】（若查無合規資料請填【資料不足，無法確認】。請強制使用 Markdown 語法排版！）
    1. 主要業務：請強制以「- **[業務名稱]**：[具體說明]」的條列格式列出現行產品。
    2. 市場規模：【嚴格限制格式】只能輸出「產值數字 + 機構名稱」。格式強制為：「- **預估產值**：根據 [機構名稱] 預估，[年份]市場規模將達 [數字]」。(嚴禁在此寫無數據之長篇大論！請將宏觀敘述移至第5點)
    3. 價值鏈：請強制以「- **[上/中/下游]**：[代表廠商與說明]」的格式精準定位。
    4. 競爭對手：請以「- **[公司名稱]**」的條列格式列出實質競爭者。
    5. 驅動因子：請強制以「- **[趨勢重點標題]**：[詳細背景說明]」的條列格式，陳述未來成長趨勢與宏觀環境背景。(所有產業介紹請歸納於此)
    """
