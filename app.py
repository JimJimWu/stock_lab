...
# ==========================================================================
    # 💥 【V2.2 升級：雙軌分流 + 各欄位獨立防護鎖】
    # 解決 AI 恐慌症，確保找得到的如實填寫，找不到的才單獨留白
    # ==========================================================================
    summary = f"""請以頂尖量化分析顧問的身份，精準提煉「{company_name}」的產業百科。
    【各欄位獨立判定鐵律】（請逐一檢視以下 5 點。若某單一欄位查無合規資料，請「僅在該欄位」填入【資料不足，無法確認】，絕對不可跨欄位湊數，也不可因單一欄位失敗而放棄其他欄位！）
    1. 主要業務【嚴格事實】：僅限確切已知的現行產品與業務，嚴禁推測詞。
    2. 市場規模【宏觀預估】：必須包含整體市場產值數字，且「強制標註機構來源(如MIC, TrendForce)」。(若查無機構來源，此欄位直接判定資料不足)
    3. 價值鏈【嚴格事實】：明確指出該公司在上、中、下游的具體定位。
    4. 競爭對手【嚴格事實】：僅列出具備實質競爭關係的國內外上市櫃公司確切名稱。
    5. 驅動因子【宏觀預估】：基於法說會或機構共識的未來成長趨勢與關鍵技術。
    """
