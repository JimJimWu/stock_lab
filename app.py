...
def auto_update_industry_db(sid):
    ...
    # ==========================================================================
    # 💥 【V2.0 升級：導入雙軌分流 AI 提示詞】
    # 嚴格區分「硬性事實」與「宏觀預估」，徹底解決中小型股資料空白問題
    # ==========================================================================
    summary = f"""請以頂尖量化分析顧問的身份，精準提煉「{company_name}」的產業百科。
    【分流審查鐵律】（若搜尋結果不符以下各區塊條件，請直接輸出「【資料不足，無法確認】」，嚴禁無中生有）
    1. 個股主要業務【嚴格事實】：僅限確切已知的現行產品與業務，嚴禁「有望、預計」等推測詞。
    2. 產業市場規模【宏觀預估】：允許引用知名研究機構(如MIC, Gartner)對未來的預測數值，但「必須明確標註機構來源」。
    3. 產業價值鏈【嚴格事實】：明確指出該公司在上、中、下游的具體定位。
    4. 相關競爭對手【嚴格事實】：僅列出具備實質競爭關係的國內外上市櫃公司確切名稱。
    5. 產業驅動因子【宏觀預估】：允許列出基於法說會或機構共識的未來成長趨勢與關鍵技術。
    """
    # ==========================================================================
