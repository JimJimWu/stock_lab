...
# ==========================================================================
    # 💥 【V3.0 終極鎮壓版：強制 JSON 範本與 Markdown 鑲嵌】
    # 放棄自然語言說服，直接用 JSON 藍圖物理鎖死 AI 的輸出格式與內容
    # ==========================================================================
    summary = f"""請以頂尖量化分析顧問的身份，精準提煉「{company_name}」的產業百科。
    【最高系統鐵律：強制 JSON 輸出模板】
    你必須嚴格回傳以下的 JSON 格式！請照抄 Key 值，並將你的分析結果依照「指定的 Markdown 符號」填入 Value 中。
    (警告：若某欄位查無合規資料，該 Value 請直接填寫「【資料不足，無法確認】」，嚴禁自行寫作文湊數！)

    {{
        "company_brief": "- **現行業務**：[具體說明]\\n- **主力產品**：[具體說明]",
        "overview": "- **預估產值**：根據 [調研機構名稱] 預估，[年份]市場規模將達 [具體數字]。(嚴禁在此撰寫無數據之產業簡介，違者直接判定失敗)",
        "value_chain": "- **上游**：[代表廠商與說明]\\n- **中游**：[代表廠商與說明]\\n- **下游**：[代表廠商與說明]",
        "competitors": "- **[對手名稱1]**\\n- **[對手名稱2]**\\n- **[對手名稱3]**",
        "drivers": "- **[趨勢重點標題1]**：[詳細背景說明]\\n- **[趨勢重點標題2]**：[詳細背景說明]"
    }}
    """
